...
# will wait until the dynamodb is up to create tables
while ! nc -z localhost 8000; do sleep 1; done

# create message jornal table
~/.local/bin/aws dynamodb create-table --table-name dynamo_message_journal \
...
~/.local/bin/aws dynamodb create-table --table-name dynamo_message_snapshots \
...
# keep the docker conteiner alive
